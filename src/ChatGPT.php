...
class ChatGPT extends stdClass{
    ...
    if ($this->settings !== null && isset($this->settings['model'])) {
        $this->model = $this->settings['model'];
    } else {
        throw new Exception("Model parameter is not set.");
    }
    ...
    public function db_analysis(string $user_message) {
    $table_descriptions = getAllTableDescriptions();

    $prompt = "Based on descriptions of tables in a database, which tables do you need to answer this question '" . $user_message . "'? Here are table names and their descriptions: '" . $table_descriptions . "'. Write the answer in the following form: gtg_products, table3, table11. I don't need any additional output, just names of tables separated by commas. Don't forget that I need exactlt the same table names, and if the name has the underscores (_) don't remove underscores. If there are no useful tables write: No information";
...
public function umessage(string $user_message) {
    global $conn;
    $selectedTablesString = $this->db_analysis($user_message);
    $selectedTables = explode(', ', $selectedTablesString);
        ...
        try {
            ...
            foreach ($data as $row) {
                foreach ($row as $column => $value) {
                    $business_data_string .= "$column: $value, ";
                }
                $business_data_string = rtrim($business_data_string, ", ") . "; ";
                $message = [
			"role" => "user",
			"content" => "Here is the information that you need to use: " . $business_data_string . " Based on this information answer the following question: " . $user_message,
		    ];
            }
        } catch(PDOException $e) {
            $message = [
			"role" => "user",
			"content" => "This is a general question. Answer the following question like you would normally do: " . $user_message,
		    ];
        }
    }
...
public function response(
    ...
    error_log("Sending request to ChatGPT API with the following parameters: " . json_encode($params));
    ...
    // Log the response
    error_log("Received response from ChatGPT API: " . $curl_exec);
